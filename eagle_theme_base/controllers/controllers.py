...
class WebsiteSale(main.WebsiteSale):
    ...
    def ks_getShopValues(self, page=0, category=None, search='', ppg=False, **post):
        ...
        values = {
            ...
            'breadcumb_shop': request.env['eagle_theme_ecomm.ks_breadcumb'].search([]).ks_breadcumb_image_url,
        }
        if category:
    ...
    def shop_load_more(self, **post):
        ...
        values = self.ks_getShopValues(page=1, category=ks_cate, search=ks_search, ppg=ppg, min=ks_min,
                                    max=ks_max, **post)
        shop_products = request.env['ir.ui.view'].render_template("eagle_theme_ecomm.products_infinite_loader", values)
        if request.website.viewref('website_sale.products_list_view').active:
            shop_products = request.env['ir.ui.view'].render_template("eagle_theme_ecomm.products_list_view_load_more",
                                                                      values)
            list_view=True
        return ({
